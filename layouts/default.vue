<template>
    <div class="min-h-screen bg-gradient-to-b from-gray-50 to-white">
                ...
                <div class="flex justify-between h-14 sm:h-16">
                    ...
                    <div class="flex items-center">
                        <NuxtLink to="/" class="flex items-center space-x-2">
                            ...
                            <span
                                class="text-base sm:text-xl font-bold bg-gradient-to-r from-blue-600 to-blue-800 bg-clip-text text-transparent truncate max-w-[200px] sm:max-w-none">
        ...
        <Teleport to="body">
            <!-- 오버레이 -->
            <Transition enter-class="opacity-0" enter-to-class="opacity-100"
                enter-active-class="transition-opacity duration-200" leave-class="opacity-100"
                leave-to-class="opacity-0" leave-active-class="transition-opacity duration-200">
                <div v-if="isMenuOpen" class="fixed inset-0 bg-black/20 z-40" @click="isMenuOpen = false"></div>
            </Transition>
            ...
            <Transition enter-active-class="transition duration-200 ease-out"
                ...
                <div v-if="isMenuOpen"
                    class="fixed top-14 sm:top-16 inset-x-0 bg-white border-b border-gray-200 pb-3 shadow-lg z-50 sm:hidden">
                    <div class="space-y-1 px-4">
                        <NuxtLink v-for="item in navigation" :key="item.to" :to="item.to"
                            class="block px-3 py-2 rounded-lg text-base font-medium transition-all duration-200" :class="[
                                $route.path === item.to
                                    ? 'bg-blue-100 text-blue-700'
                                    : 'text-gray-600 hover:bg-gray-100'
            ...
            <div class="max-w-7xl mx-auto py-4 sm:py-6 px-3 sm:px-6 lg:px-8">
                <div class="flex flex-col items-center space-y-2">
                    ...
                    <div class="flex items-center space-x-2">
                        <svg class="w-5 sm:w-6 h-5 sm:h-6 text-blue-600" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor">
                            <circle cx="12" cy="12" r="10" stroke-width="2" />
                            <path stroke-width="2" d="M12 6v6l4 4" />
                        </svg>
                        <span class="text-xs sm:text-sm font-semibold text-gray-900">Unix Timestamp Converter</span>
                    </div>
                    <p class="text-xs sm:text-sm text-gray-500 text-center px-4">
